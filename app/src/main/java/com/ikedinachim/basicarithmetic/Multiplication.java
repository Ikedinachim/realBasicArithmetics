...
//                    timerProgressBar.setProgress(30);//hj
    ...
    public int[] generateNumbers(int numOfQues){
        ...
        if (numOfQues <= 10){
            num1  = random.nextInt(5)+5; // random number from 12 to 12+18
            num2 = random.nextInt(5) + 5;
        ...
        if (numOfQues <= 30){
            num1  = random.nextInt(25)+5;
            num2 = random.nextInt(25)+5;
        ...
        timerProgressBar.setMax(max);
        timerProgressBar.setProgress(currentProgressTimer);
        return new int[]{random.nextInt(30)+5, random.nextInt(30)+5};
    }
    ...
    public void storeHighScore(int highScore){
        ...
        store = getSharedPreferences("MULTIPLY", 0);
        SharedPreferences.Editor editor = store.edit();
        editor.putInt("MULTIPLY",highScore);
        editor.commit();

    }
    public int retrieveHighScore(){
        store = getSharedPreferences("MULTIPLY", 0);
        int hscore = store.getInt("MULTIPLY", 0); //0 is the default value
        return  hscore;
    }
}
